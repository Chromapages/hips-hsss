...
| **Rate Limiting** | Per-endpoint (Clerk) | Implied at API Gateway | **Spec wins + extended** — explicit per-endpoint limits with API gateway enforcement | |
...
│  Routes       │  │  (Clerk + anon   │  │  Keyword     │  │  Homework gen    │
...
### Auth Security
- Clerk JWT verified server-side on every commerce API route
...
**Auth:** Clerk commerce layer + anonymous session token layer
