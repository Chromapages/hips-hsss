...
describe('POST /api/v1/sessions/book', () => {
  // Happy paths
  it('201: books session with valid serviceId, scheduledAt, and Firebase JWT')
...
describe('POST /session/v1/token', () => {
  it('201: issues token for CONFIRMED session with valid Firebase JWT')
...
test('complete session booking flow', async ({ page }) => {
  // 1. Sign up / sign in via Firebase
  await page.goto('/sign-up')
  // ... Firebase sign-up steps
...
describe('role verification — DB check, not JWT-only', () => {
  ...
  it('valid Firebase JWT with no corresponding User in Commerce DB returns 401')
  it('valid Firebase JWT where User.role !== JWT.role uses DB role (not JWT role)')
