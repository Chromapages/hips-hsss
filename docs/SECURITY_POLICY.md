...
| Production (frontend / API routes) | Vercel Project Settings (encrypted env vars) | Clerk, Stripe, Resend, Supabase keys |
...
| `CLERK_SECRET_KEY` | Commerce Next.js API routes | Vercel Project Settings | Per Clerk recommendation |
...
- Clerk JWT is short-lived; rotation handled by Clerk

**Layer 2 — Session (Anonymous Tokens)**
- Issued by Auth Service on session start
- Linked to `session_id` only — never to a Clerk user ID, email, or real name
...
- All `/api/v1/` endpoints require a valid Clerk JWT unless explicitly listed as public
...
Validation:     Clerk JWT must be valid before token is issued
...
model AuditEvent {
  ...
  actorId     String   -- Clerk user ID (hashed for non-admin actors)
...
```
Metric:     Clerk webhook auth failure events
Threshold:  ≥ 20 failures in 5 minutes
Action:     SNS → Slack #security-alerts
Response:   P1 — possible credential stuffing on commerce auth. Review Clerk dashboard.
```
...
2. Out-of-scope systems (e.g., Clerk infrastructure — managed by Clerk)
...
| **P1 — High** | Payments down; auth completely broken; crisis protocol fails; vault anomaly alarm | 15 minutes | Stripe webhooks not processing; Clerk outage; vault access spike alarm |
...
2. If Clerk incident: communicate ETA; no action
3. Verify `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY` and `CLERK_SECRET_KEY` in Vercel
4. If env vars correct and Clerk healthy: review `middleware.ts` for recent changes
...
**Out of scope:** Clerk infrastructure, Stripe infrastructure, Railway infrastructure, social engineering attacks, physical attacks
