...
| Auth layer | Firebase (commerce layer) |
...
Your Firebase JWT contains `role: FACILITATOR`. This role is verified against the database on every admin operation — the JWT claim alone is not sufficient for sensitive actions.
...
- Which Firebase account corresponds to which session
...
After signing in at `hips.foundation/sign-in` with your Firebase account:
...
- [ ] Receive Firebase invitation email from `H.I.P.S. Foundation` — create your account
...
- [ ] Sign in to Cal.com via Firebase OAuth (same credentials as platform login)
...
1. Your Firebase account is removed from the H.I.P.S. organization — you lose all platform access
