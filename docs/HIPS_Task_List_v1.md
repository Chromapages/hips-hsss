...
**Stack:** Next.js 14 (App Router) + TypeScript + Tailwind | PostgreSQL + Prisma | Stripe | Firebase | NestJS (session services) | Supabase Storage | Vercel + Railway + HIPAA-capable cloud
...
| 0.9 | Set up Firebase application + configure publishable/secret keys | Infra | S | None | Configure allowed redirect URLs |
...
| 1A.2 | Write User model + Role enum migration | DB | S | 1A.1 | UUID PK, FirebaseId unique, role, soft delete |
...
| # | Title | Layer | Effort | Dependencies | Notes |
|---|---|---|---|---|---|
| 2.1 | Configure Firebase middleware for commerce routes | BE | M | 0.9, 1A.2 | Protect all `/app/*` and `/api/v1/*` routes except public ones |
| 2.2 | Firebase webhook: on sign-up → create User record in commerce DB | BE | M | 2.1, 1A.2 | `user.created` webhook event; idempotent |
...
| 2.6 | Write sign-in page (Firebase-hosted or custom) | FE | S | 2.1 | `/sign-in` — Firebase `<SignIn />` component |
| 2.7 | Write sign-up page | FE | S | 2.1 | `/sign-up` — Firebase `<SignUp />` component |
