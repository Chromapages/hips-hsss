...
| P1 — High | Payments down; auth completely broken; crisis protocol fails | 15 minutes | Stripe webhooks not processing; Clerk outage |
...
1. Check Clerk status: https://status.clerk.com
   - If Clerk incident: communicate ETA; no action

2. Check Clerk Dashboard for error logs

3. Verify NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY and CLERK_SECRET_KEY in Vercel env

4. If env vars are correct and Clerk is healthy — check middleware.ts for recent changes
...
| Clerk Dashboard | clerk.com + MFA | Tech lead |
