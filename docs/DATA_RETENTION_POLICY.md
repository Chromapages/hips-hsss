...
This policy covers all data collected, processed, or stored by the H.I.P.S. platform across all three data stores (Identity Vault, Session DB, Commerce DB), object storage (Supabase), logs (CloudTrail, CloudWatch), and third-party processors (Clerk, Stripe, Resend, Supabase, LiveKit).
...
| Clerk auth logs | Clerk (third-party) | Per Clerk policy | Clerk managed | No session content |
...
Session data is anonymous by design — no session record contains a real name, email, or Clerk user ID. Retention is governed by operational need, not by identity protection (since no identity is present).
...
|---|---|---|---|
| **Clerk** | Email, name (for account creation), role | Per Clerk Privacy Policy | Delete Clerk user on account deletion request (via Clerk API) |
...
1. Delete Clerk user account via Clerk Management API
...
Log:        AuditEvent { eventType: 'ANNUAL_HARD_DELETE_JOB', operator: clerkId, count: N }
```
...
|---|---|---|
| Commerce account (name, email in Clerk) | ✅ Yes | Immediately (soft-delete); hard delete after 30 days |
...
2. **Identity verification:** participant must be authenticated (Clerk session) or verify via emailed confirmation link
...
- [ ] Review Clerk data retention settings — confirm alignment with this policy
