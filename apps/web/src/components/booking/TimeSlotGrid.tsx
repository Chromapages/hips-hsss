  ...
  if (!date) {
    return (
      <section className="rounded-[2rem] border border-white/5 bg-zinc-950/50 p-10 flex flex-col items-center justify-center text-center min-h-[300px] border-dashed">
        <div className="w-12 h-12 rounded-full bg-surface/5 flex items-center justify-center mb-4">
          <Calendar className="w-6 h-6 text-text" />
        </div>
        <p className="text-sm font-bold uppercase tracking-widest text-text">
  ...
  if (loading) {
    return (
      <section className="rounded-[2rem] border border-white/5 bg-zinc-950 p-8">
        <div className="h-6 w-32 bg-surface/5 rounded-full mb-8 animate-pulse" />
        <div className="grid gap-3 sm:grid-cols-2">
          {[1, 2, 3, 4].map(i => (
            <div key={i} className="h-14 rounded-2xl bg-surface/5 animate-pulse" />
          ))}
        </div>
      </section>
    );
              ...
              <button
                ...
                className={[
                  ...
                  isActive 
                    ...
                    : "bg-surface/5 border-white/5 text-text hover:border-primary/50 hover:bg-surface/10"
                ].join(" ")}
                type="button"
              >
                {timeStr}
