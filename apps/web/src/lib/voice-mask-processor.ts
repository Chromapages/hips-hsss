...
export function createVoiceMaskProcessor(
  ...
  const processor: TrackProcessor<Track.Kind.Audio, AudioProcessorOptions> = {
    ...
    async init(opts) {
      ...
      // Validate AudioContext sample rate matches the track's expected rate to avoid
      // pitch/speed artifacts in the Hanning-window pitch shifter (timing is sensitive
      // to buffer alignment at the half-buffer crossfade boundary).
      // In practice, browser-created AudioContexts for MediaStream tracks use the
      // system sample rate (typically 48kHz) which matches most hardware. The risk
      // is non-standard sample rate hardware (some USB microphones, certain Safari
      // configurations) where the pitch and reverb timing would be slightly off.
      const ctxSampleRate = opts.audioContext.sampleRate;
      if (ctxSampleRate !== 48000 && ctxSampleRate !== 44100) {
        console.warn(
          `[VoiceMaskProcessor] Non-standard AudioContext sample rate: ${ctxSampleRate}Hz. ` +
          'Pitch and reverb timing may be slightly off. This is non-fatal.',
        );
      }
