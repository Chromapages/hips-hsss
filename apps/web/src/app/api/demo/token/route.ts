...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!process.env.LIVEKIT_API_KEY || !process.env.LIVEKIT_API_SECRET) {
      // In production this is unreachable because the check above exits with 500,
      // but Next.js build may reference this file server-side in environments
      // where the vars are not yet set — fail fast so the error is visible.
      throw new Error(
        'LIVEKIT_API_KEY and LIVEKIT_API_SECRET must be set in the server environment. ' +
        'Add them to .env.local (non-NEXT_PUBLIC) or your platform secrets manager.'
      );
    }

    const livekitRegion = process.env.LIVEKIT_REGION;
    const livekitUrl = livekitRegion
      ? `wss://${livekitRegion}-api.livekit.cloud`
      : (process.env.NEXT_PUBLIC_LIVEKIT_URL || process.env.NEXT_PUBLIC_LIVEKIT_WS_URL || 'wss://demo.livekit.cloud');
    const roomService = new RoomServiceClient(livekitUrl, apiKey, apiSecret);
    await ensureDemoRoomExists(roomService);

    const expiresAt = new Date(Date.now() + DEMO_TTL_SECONDS * 1000);

    // Unique per-visit identity — random UUID only (no IP in identity string to avoid collision across NAT)
    const visitorIdentity = `demo-${crypto.randomUUID().slice(0, 12)}`;
    ...
    at.addGrant({
      roomJoin: true,
      room: DEMO_ROOM_NAME,
      canPublish: true,  // mic/camera publish allowed in demo sandbox
      canSubscribe: true,
      canPublishData: false,  // data channel (chat/messages) disabled for demo sandbox
    });
